...
function iniciarTudo() {
    ...
    // Mensagem personalizada com plural para casais
    const mensagemEl = document.getElementById('mensagem-personalizada');
    if (tipo === 'casal_padrinho') {
        mensagemEl.textContent = `${nome}, vocês são essenciais para nós!`;
    } else {
        mensagemEl.textContent = `${nome}, você é essencial para nós.`;
    }
...
function revelarTexto() {
    ...
    const frases = {
        ...
        'casal_padrinho': "Vocês aceitam ser nossos padrinhos de casamento?" // ← frase completa no plural
    };
